...
"""Code for testing the prediction model."""
    #训练和预测模型代码
import numpy as np
...
from output_image import output_gif
...
flags.DEFINE_string('data_dir', 'grap_tfrecord', 'directory containing data.') #包含数据的目录
...
flags.DEFINE_string('pretrained_model', 'tmp/data/model12002',
...
def prediction():
  ...
  with tf.variable_scope('model', reuse=None) as training_scope:
    images, actions, states = build_tfrecord_input(training=False,vil=False)
  ...
  psnr,cost, _, summary_str,images,gen_images= sess.run([model.psnr_all,model.loss, model.train_op, model.summ_op,model.images,model.gen_images],
